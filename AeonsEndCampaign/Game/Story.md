# Story: The Whisper Below

> This file contains a one-paragraph summary of each chapter. Read this first to know where things stand. For the full narrative, open the corresponding chapter in `Chapters/`.

---

## Prologue: The Quiet Weeks

For three weeks, Gravehold knew peace — until miners started vanishing from the Deepvein tunnels and the Silver Rift. A sole survivor, Kellan Voss, emerged clutching a pulsing black crystal, speaking of something awake beneath the city. The tunnels were sealed. The Breach Mages were summoned. Something is waiting below.

→ Full narrative: `Chapters/Prologue.md`

---

## Chapter 01: The Whisper Below

The mages answer the call just as Rageborne marches on the East Gate. Kadir, Adelheim, and Xaxos (AE) hold the line while Brama tends a fever outbreak in the Lower Markets. Kadir strikes the killing blow at the cost of her own exhaustion, earning the title "The Unbroken." In the quiet hours after the battle, each mage wrestles with what they gave and what they lost — Adelheim walks the walls alone, Kadir dreams of green light, Xaxos burns with unspent fire, and Brama works through the night in the fever wards.

→ Full narrative: `Chapters/Chapter 01 - The Whisper Below.md`
→ Full mission brief: `Missions/Mission 01 - What the Dark Attracts.md`
→ Session report: `Sessions/S01 - What the Dark Attracts.md`

---

## Chapter 02: The City Divided

The Council chose to defend the Outer Settlements from the Carapace Queen's swarm. Brama, Jian, and Lash delivered a comprehensive victory — Brama struck the killing blow with two Scrying Bolts, and the mages never reached the Queen's tier 3 cards. But while they fought, the unchosen threat escalated: the Deepvein signal intensified, Kellan Voss's condition worsened, and the drake in the Warrens remains unsaved. In the quiet hours after, each mage wrestles with what they discovered about themselves — Brama with the weight of killing, Jian with a new hunger for battle, Lash with a fire that never got to burn, and Kadir with the growing awareness that something below is watching.

→ Full narrative: `Chapters/Chapter 02 - The City Divided.md`
→ Full mission brief: `Missions/Mission 02 - The City Divided.md`
→ Session report: `Sessions/S02 - The City Divided.md`

---

*More chapters will be added as the campaign progresses.*